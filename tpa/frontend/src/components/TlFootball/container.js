...
import TlFootball from "./presenter";
...
TLLogicalSeatLayerBase.prototype.processSeats = async function(seatInfo){
	var prevx = 0;
	var prevy = 0;
		//console.log(x,y,seatInfo);
	...
	for (var idx = seatInfo.length - 1; idx >= 0; idx--) {
		...
		var gradeInfo = gradeMap[seat.gradeId];
	...
	await new Promise((resolve, reject) => {
		tileImage.onload = function() {
		TLDomInfo.getInstance().context.drawImage(tileImage, x, y);
...
var seatInfo = [];
async function  gotoSeatPlan (blockObj,blockId,gradeId) {
	seatInfo = await getSeatInfo(blockId,gradeId);
...
function getSeatTooltip (seat) {
	var tooltip = ["[ " + gradeMap[seat.gradeId].name + "] "];
...
async function processTiles(seatInfo) {
	...
	TLLogicalSeatLayer.getInstance().processSeats(seatInfo);
	TLDomInfo.getInstance().areaCanvas.style.display = 'none';
	TLDomInfo.getInstance().canvas.style.display = 'block';
}
...
class Container extends Component {
  ...
  static propTypes = {
    getFootballBlockObj : PropTypes.func.isRequired,
    blockobj : PropTypes.array.isRequired,
  };
  ...
  btnGradeClick = (e,gradeId) => {
	//console.log(this.state.blockObj,gradeId);
	const filteredBlocksA = this.filterBlocksByGrade(this.state.blockObj.blocks, gradeId);
	if(filteredBlocksA.length===1){
		this.setState({
			selectBlock : filteredBlocksA[0]
		})
		selectSeats = [];
		this.setState({
			selectSeat : []
		})
		gotoSeatPlan(filteredBlocksA[0].block,filteredBlocksA[0].block.blockId,gradeId);
	}
	else{
		//console.log(filteredBlocksA);
		this.setState({
			selectBlockMap : filteredBlocksA
		})
	}
  }
  btnBlockClick = (e,block,gradeId) => {
	this.setState({
		popup : false,
	})
	//console.log(block,gradeId,"dd");
	selectSeats = [];
	this.setState({
		selectSeat : []
	})
	gotoSeatPlan(block,block.blockId,gradeId);
  }
  stateChange = (name,change) => {
	this.setState({
		[name] : change,
	})
  }
  componentWillReceiveProps = nextProps => {
    if(nextProps.blockobj ){
      if(this.state.blockObj.length==0){
        this.setState({
			blockObj : nextProps.blockobj,
			gradeMap : nextProps.blockobj.grade
        })
		blockInfo = nextProps.blockobj.blocks;
		gradeMap = nextProps.blockobj.grade;
		blockBgInfo.imageUrlThumnail = nextProps.blockobj.imageUrlThumnail;
		TLSeatBackgroundLayer.getInstance().imageUrl = nextProps.blockobj.imageUrlThumnail.split("images/")[0] + "images/" +  nextProps.blockobj.eng + "/seat_{x}_{y}.png";
  ...
  openPopup = () => {
    ...
    // 일정 시간(예: 1000 밀리초 = 1초) 후에 팝업 외부 클릭 이벤트 활성화
    setTimeout(() => {
		popupclickEnabled = true;
    }, 50);

    // 여기서 팝업을 열도록 작업을 진행할 수 있습니다.
  };
  ...
  //https://image.toast.com/aaaaab/ticketlink/TKL_CONCERT/new_twins_seat_23(0512)_4
  componentDidMount() {
    const {getFootballBlockObj } = this.props;
	const lastSegment = window.location.pathname.split("/").pop();
	getFootballBlockObj(lastSegment)
	document.title = 'T사 축구('+lastSegment+') 티켓팅 연습';
  ...
  render() {
    return (
      <TlFootball startTimer={this.startTimer} clickReplay={this.clickReplay} clickNext={this.clickNext} allView={this.allView} {...this.state} {...this.props} popupClose={this.popupClose} btnBlockClick={this.btnBlockClick} selectGrade={this.selectGrade} btnGradeClick={this.btnGradeClick} />
    );
  }
}
